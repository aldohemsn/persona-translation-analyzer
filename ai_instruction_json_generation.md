...
- **Contextual Segments:** Crucially, the `source_segment`, `draft_segment`, and `revision_segment` fields must contain the **full, untruncated sentence(s)** where the issue is located. **Do not use ellipses (...) or otherwise shorten these strings**, as the full text is required for downstream data processing that is intolerant to any truncation. For example, instead of just `"起源地之一，且在技术传承...佐证"`, the full, contextually complete segment is `"起源地之一，且在技术传承、完善和传播方面，可以得到实物、技术分析结果和文献佐证"`.
